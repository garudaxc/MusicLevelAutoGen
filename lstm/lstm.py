...
#C wrappers for speeding up training in LSTM
...
void prodplus(int n,double out[n],double u[n],double v[n]) {
    for(int i=0;i<n;i++) {
        out[i] += u[i]*v[i];
    }
}
...
void sumprod(int r,int n,double a[n],double u[r][n],double v[r][n]) {
    for(int i=0;i<n;i++) {
        double total = 0.0;
...
class LSTM(Network):
    ...
    def __init__(self,ni,ns,initial=initial_range,maxlen=200):
        na = 1+ni+ns
        self.dims = ni,ns,na
    ...
    def init_weights(self,initial):
        ...
        ni,ns,na = self.dims
        # gate weights
	#WGI = Input gate weights
	#WGF = Forget gate weights
	#WGO = Output gate weights
	#WCI = Cell input weights
        for w in "WGI WGF WGO WCI".split():
            setattr(self,w,randu(ns,na)*initial)
            setattr(self,"D"+w,zeros((ns,na)))
        # peep weights
	#WIP = Input peep weights
	#WFP = Forget peep weights
	#WOP = Output peep weights
	...
	#source = inputs to the block ( inputs from input layer and recurrent inputs)
	#state = value stored in the cell inside the block
	#output = output of the lstm block
	#cix = cell input summation
	#ci = cell input activation
	#gix = input gate summation
	#gi = input gate activation
	#gox = output gate summation
	#go = output gate activation
	#gfx = forget gate summation
	#gf = forget gate activation
    ...
    def forward(self,xs):
        ...
        #Input from previous activation
            self.source[t,1+ni:] = prev
        #Find the weighted sum at Input,Forget,Output gates and Cell input
        ...
        #Input, Forget gate and Cell input activations
            self.gi[t] = ffunc(self.gix[t])
            self.gf[t] = ffunc(self.gfx[t])
            self.ci[t] = gfunc(self.cix[t])
        #value stored in cell is sum of (input cell activation at t*Input gate activation at t
        # + (forget gate activation at t * value stored in cell at t-1)
    ...
    def backward(self,deltas):
        ...
        self.DWIP = nsumprod(self.gierr[1:n],self.state[:n-1],out=self.DWIP)
        self.DWFP = nsumprod(self.gferr[1:n],self.state[:n-1],out=self.DWFP)
        self.DWOP = nsumprod(self.goerr[:n],self.state[:n],out=self.DWOP)
        self.DWGI = nsumouter(self.gierr[:n],self.source[:n],out=self.DWGI)
        self.DWGF = nsumouter(self.gferr[1:n],self.source[1:n],out=self.DWGF)
        self.DWGO = nsumouter(self.goerr[:n],self.source[:n],out=self.DWGO)
        self.DWCI = nsumouter(self.cierr[:n],self.source[:n],out=self.DWCI)
...
def NetFactory(directionality,layer_sizes):
    """A Factory method which returns the required architecture of lstm network"""
    assert len(layer_sizes)>=3
    l=len(layer_sizes)-1
    sizes=[None]*l
    for i in range(l):
        sizes[i] = (layer_sizes[i],layer_sizes[i+1])
    if(directionality=='unidirectional'):
        nets=[None]*l
        for i in range(l-1):
            nets[i] = LSTM(sizes[i][0],sizes[i][1])
        if layer_sizes[-1]==1:
            nets[-1] = Logreg(sizes[-1][0],sizes[-1][1])
        else:
            nets[-1] = Softmax(sizes[-1][0],sizes[-1][1])
        return Stacked(nets)
    elif(directionality=='bidirectional'):
        nets_f=[None]*(l-1)
        nets_b=[None]*(l-1)
        for i in range(l-1):
            nets_f[i] = LSTM(sizes[i][0],sizes[i][1])
            nets_b[i] = LSTM(sizes[i][0],sizes[i][1])
        net_f = Stacked(nets_f)
        net_b = Reversed(Stacked(nets_b))
        bidi = Parallel(net_f,net_b)
        if layer_sizes[-1]==1:
            logreg = Logreg(2*sizes[-1][0],sizes[-1][1])
        else:
            logreg = Softmax(2*sizes[-1][0],sizes[-1][1])
        stacked = Stacked([bidi,logreg])
        return stacked
...
class SeqRecognizer:
    ...
    def __init__(self,directionality,ninput,nstates,noutput=-1,codec=None,normalize=normalize_nfkc):
        self.Ni = ninput
        ...
        t.insert(0,ninput)
        t.append(noutput)
        self.lstm = NetFactory(directionality,t)
